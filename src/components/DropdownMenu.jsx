    ...
    <Menu as="div" className="relative inline-block text-left">
      ...
      >
        <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md border border-zinc-400 dark:border-zinc-700 bg-orange-50 dark:bg-zinc-800 shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none divide-zinc-400 dark:divide-zinc-700">
          <div className="py-1">
            <div className="px-3 py-2 uppercase font-bold text-xs lg:text-xl font-mplus">
              Categories
            </div>
            {pages.map(page => {
              return (
                <Menu.Item key={page.name}>
                  {({ active }) => (
                    <a
                      href={page.href}
                      className={`${active ? "bg-orange-100 dark:bg-zinc-700" : ""
                        } group flex rounded-md items-center w-full px-2 py-2 text-sm font-mplus`}
                    >
                      {page.name}
                    </a>
                  )}
